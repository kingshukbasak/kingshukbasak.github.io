(function main() {
    ...
    //JSON data
    var data = {
        parent: { element: "div", className: "maindiv", id: "maindiv" },
        child1: { parent: { element: "div", id: "labeldiv" },
            child1: { parent: { element: "label", className: "label", innerHTML: "Basic Calculator", id: "label", style: { marginRight: "10px"}} },
            child2: { parent: { element: "label", className: "label", innerHTML: "Date Calculator", id: "label", style: { marginRight: "10px"}} },
            child3: { parent: { element: "label", className: "label", innerHTML: "Mortgage Calculator", id: "label", style: { marginRight: "10px"}} }
        },
        child2: { parent: { element: "div", id: "radiodiv", style: { position: "absolute", top: "38px"} },
            child1: { parent: { element: "input", className: "radio", type: "radio", name: "select", id: "basic", style: { marginRight: "20px", marginLeft: "50px" }, addEventListener: ["click", basicCalculator]} },
            child2: { parent: { element: "input", className: "radio", type: "radio", name: "select", id: "datecal", style: { marginRight: "20px", marginLeft: "50px" }, addEventListener: ["click", dateCalculator]} },
            child3: { parent: { element: "input", className: "radio", type: "radio", name: "select", id: "mortgage", style: { marginRight: "20px", marginLeft: "50px" }, addEventListener: ["click", mortgageCalculator]} }
        }
    };

    //parsing the JSON data
    function parseData(object, parent) {
        var i, j, element, k;
        for (i = 0; i < Object.keys(object).length; i++) {
            var key = Object.keys(object)[i];
            if (key == 'parent') {
                element = document.createElement(object[key].element);
                for (j = 1; j < Object.keys(object[key]).length; j++) {
                    var keyinner = Object.keys(object[key])[j];
                    if (keyinner == 'style')
                        for (k in object[key][keyinner])
                            element[keyinner][k] = object[key][keyinner][k];
                    else if (keyinner == 'addEventListener') {
                        var event = object[key][keyinner][0];
                        var func = object[key][keyinner][1];
                        element[keyinner](event, func);
                    }

                    else
                        element[keyinner] = object[key][keyinner];
                }
                parent.appendChild(element);
            }
            else if (key != 'parent') {
                parseData(object[key], element);
            }
        }
    }

    //calling the function
    parseData(data, mainBody);
